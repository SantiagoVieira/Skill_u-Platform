...
export default function AdminPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total:    all.length,
      pending:  all.filter(m => !m.curation_status || m.curation_status === "pending").length,
      approved: all.filter(m => m.curation_status === "approved").length,
      rejected: all.filter(m => m.curation_status === "rejected").length,
    };
  }, [materials]);
  ...
  const TABS: { key: FilterTab; label: string; color: string }[] = [
    ...
    { key: "approved", label: "Aprobados",  color: "#16a34a" },
    { key: "rejected", label: "Rechazados", color: "#dc2626" },
    ...
    <>
      ...
      <div style={{ display: "flex", height: "calc(100vh - 58px)" }}>
        ...
        <aside style={{
          ...
          background: "var(--white)", padding: "20px 0", flexShrink: 0,
          overflowY: "auto",
        }}>
          {/* Stats */}
...
function MaterialRow({
  material: m, isSelected, onClick, onApprove, onReject,
}: {
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
            ...
            {m.price === 0
              ...
              : <span style={{ fontSize: 10, background: "var(--gray-100)", color: "var(--gray-600)", borderRadius: 4, padding: "1px 6px", fontWeight: 500 }}>${m.price.toLocaleString("es-CO")} COP</span>
            }
          </div>
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <span style={{ fontSize: 12, fontWeight: 700, color: "var(--gray-500)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
          Detalle
        </span>
      ...
      {/* Título */}
      <h2 style={{ fontSize: 15, fontWeight: 700, color: "var(--gray-900)", marginBottom: 4, lineHeight: 1.4 }}>
        {m.title}
      </h2>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 12, color: "var(--gray-600)", lineHeight: 1.6, margin: 0 }}>
          {m.description || <em style={{ color: "var(--gray-400)" }}>Sin descripción</em>}
        </p>
      </div>
        ...
        {m.file_url ? (
          <a
            ...
            style={{
              ...
              padding: "10px 12px", background: "#ede9fe",
              borderRadius: 8, border: "1px solid #c4b5fd",
              color: "#5b21b6", fontSize: 12, fontWeight: 500,
              textDecoration: "none",
            }}
          >
            <FileTypeIcon type={m.file_type} />
      ...
      {status !== "pending" && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{
            ...
            border: `1px solid ${status === "approved" ? "#bbf7d0" : "#fecaca"}`,
          }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: status === "approved" ? "#166534" : "#991b1b", marginBottom: 4 }}>
              {status === "approved" ? "✅ Aprobado" : "🚫 Rechazado"}
            </div>
            {m.curation_notes && (
              <p style={{ fontSize: 11, color: status === "approved" ? "#15803d" : "#b91c1c", margin: 0 }}>
                {m.curation_notes}
              </p>
            )}
            {m.curated_at && (
              <p style={{ fontSize: 10, color: "var(--gray-400)", margin: "6px 0 0" }}>
                {new Date(m.curated_at).toLocaleString("es-CO")}
              </p>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: "var(--gray-900)", marginBottom: 4 }}>
            {isReject ? "🚫 Rechazar material" : "✅ Aprobar material"}
          </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <textarea
            ...
            placeholder={
              isReject
                ? "Ej: El documento contiene contenido que viola nuestras políticas de uso…"
                : "Ej: Excelente material, bien estructurado."
            }
            rows={4}
            style={{
              ...
              lineHeight: 1.6, color: "var(--gray-900)",
            }}
          />
        </div>
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: 11, fontWeight: 600, color: "var(--gray-500)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
      {children}
    </p>
  );
}

function Tag({ children, color = "var(--gray-600)", bg = "var(--gray-100)" }: {
  children: React.ReactNode; color?: string; bg?: string;
}) {
  return (
    <span style={{ fontSize: 10, background: bg, color, borderRadius: 4, padding: "2px 7px", fontWeight: 600 }}>
      {children}
    </span>
  );
}

function QuickBtn({ title, color, bg, onClick, children }: {
...
function FileTypeIcon({ type }: { type: string | null }) {
  const colors: Record<string, string> = {
    PDF: "#ef4444", DOC: "#3b82f6", IMG: "#10b981", VID: "#8b5cf6",
  };
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
      stroke={colors[type ?? "PDF"] ?? "#7c3aed"} strokeWidth="2" strokeLinecap="round">
